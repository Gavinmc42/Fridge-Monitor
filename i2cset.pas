...
type
  ...
  TShellCommandi2cset = class(TShellCommand)
  public
   constructor Create;
  private
...
constructor TShellCommandi2cset.Create;
begin
 {}
 inherited Create;
...
function TShellCommandi2cset.DoHelp(AShell:TShell;ASession:TShellSession):Boolean;
begin
 ...
 AShell.DoOutput(ASession,'i2csget chipaddress registeraddress registervalue');
 AShell.DoOutput(ASession,'i2cget 0x00 0x00 0x00');
...
function TShellCommandi2cset.DoInfo(AShell:TShell;ASession:TShellSession):Boolean;
begin
 ...
 Result:=AShell.DoOutput(ASession,'i2cset sets i2c chip byte registers');
end;

function TShellCommandi2cset.DoCommand(AShell:TShell;ASession:TShellSession;AParameters:TStrings):Boolean;
var
 Value:String;
 Parameter:String;
begin
 ...
 try
   ...
   if I2CDeviceStart(I2CDevice,100000) <> ERROR_SUCCESS then
    ...
    begin
     ...
     if I2CDeviceWriteWrite(I2CDevice,i2caddress,@i2cregister, 1, @i2cdata,SizeOf(Byte), Count) = ERROR_SUCCESS then
        begin
              //Value:= IntToStr(Data);
        end;
   ...
   Result:=AShell.DoOutput(ASession, 'Address= ' + IntToStr(i2caddress)+ ' Register= ' + IntToStr(i2cregister) + ' Data= ' + IntToStr(i2cdata));
 except
...
initialization
 ...
 ShellRegisterCommand(TShellCommandi2cset.Create);
